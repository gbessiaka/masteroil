...
export default function AdminFacturesPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-black text-brand-cream mb-1">Factures</h1>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-brand-cream text-sm font-mono font-bold">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-brand-cream text-sm">
                        {(invoice.order as any)?.client?.name || 'Client inconnu'}
                      </p>
                      {invoice.order_id && (
                        <Link
                          href={`/admin/commandes/${invoice.order_id}`}
                    ...
                    <td className="px-6 py-4">
                      <span className="text-brand-gold font-semibold text-sm">
                        {formatGNF((invoice.order as any)?.total_gnf || 0)}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <a
                        href={`/api/admin/invoice/${invoice.id}`}
                        target="_blank"
              ...
              {(!invoices || invoices.length === 0) && (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-zinc-500 text-sm">
                    Aucune facture générée. Créez une facture depuis le détail d&apos;une commande.
                  </td>
                </tr>
              )}
            </tbody>
