            ...
            <tbody>
              {products.map((product) => (
                <tr key={product.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4"><Badge variant={product.category === 'automobile' ? 'gold' : 'blue'}>{product.category}</Badge></td>
                  <td className="px-6 py-4">
                    <p className="text-zinc-300 text-sm font-mono">{product.viscosity || '—'}</p>
                    {product.type && <p className="text-zinc-500 text-xs mt-0.5">{typeLabel[product.type] || product.type}</p>}
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    {product.show_price && product.packagings && product.packagings.length > 0 ? (
                      <span className="text-brand-gold text-xs font-semibold">À partir de {formatGNF(Math.min(...product.packagings.map((p) => p.price_gnf)))}</span>
                    ) : (
